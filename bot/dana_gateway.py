...
class DanaPaymentGateway:
    ...
    def create_order(self, inv_id: str, amount: int, title: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        """
        if not self.merchant_id or not self.client_secret:
            # Standalone mode fallback - generates a valid payment URL / QR string for testing
            qr_content = f"https://checkout.dana.id/pay/{inv_id}?amount={amount}"
            return True, {
                "order_id": f"DANA-{inv_id}",
